...
/// <summary>
/// Интеграционные тесты TCP-сервера: поднимается настоящий сервер с TcpListenerService
/// на свободном порту, тест подключается как трекер, отправляет кадры обоих протоколов
/// и проверяет записи в SQLite.
/// </summary>
public class TcpProtocolIntegrationTests : IAsyncLifetime
{
    private TcpServerFixture _server = null!;

    // xunit.v3: IAsyncLifetime работает на ValueTask
    public async ValueTask InitializeAsync() => _server = await TcpServerFixture.StartAsync();
    ...
    public async Task Gt02_LoginGetsLoadReply_AndLocationsAreStored()
    {
        ...
        await tracker.ConnectAsync(_server.Port, ct);

        await tracker.SendAsync(Gt02Frames.Login(), ct);
        var reply = await tracker.ReceiveTextAsync(TimeSpan.FromSeconds(5), ct);
        Assert.Equal(Gt02TextProtocolParser.LoginResponse, reply);

        await tracker.SendAsync(Gt02Frames.Location(60.116, 31.3675, speed: 42, courseDegrees: 120), ct);

        var points = await _server.WaitForPointsAsync(Gt02Frames.Imei, count: 1, ct: ct);
    ...
    public async Task Gt02_TimestampIsReceptionTime()
    {
        ...
        await tracker.SendAsync(Gt02Frames.Login(), ct);
        await tracker.ReceiveTextAsync(TimeSpan.FromSeconds(5), ct);

        var before = DateTime.UtcNow;
        await tracker.SendAsync(Gt02Frames.Location(60.117, 31.368, speed: 50, courseDegrees: 90), ct);
    ...
    public async Task Gt02_MultipleLocationsAreStoredInOrder()
    {
        ...
        await tracker.SendAsync(Gt02Frames.Location(60.100, 31.300, speed: 10, courseDegrees: 0), ct);
        await tracker.SendAsync(Gt02Frames.Location(60.110, 31.320, speed: 60, courseDegrees: 180), ct);
        await tracker.SendAsync(Gt02Frames.Location(60.120, 31.340, speed: 95, courseDegrees: 359), ct);

        var points = await _server.WaitForPointsAsync(Gt02Frames.Imei, count: 3, ct: ct);
    ...
    public async Task Gt02_LocationWithoutLogin_IsNotStored()
    {
        ...
        var points = await _server.WaitForPointsAsync(Gt02Frames.Imei, count: 1, timeout: TimeSpan.FromSeconds(2), ct: ct);
    ...
    public async Task Gt02_FrameSplitAcrossPackets_IsAssembled()
    {
        ...
        await tracker.SendAsync(login[..20], ct);
    ...
    public async Task Gt06_LoginLocationHeartbeat_GetAcksAndStorePoints()
    {
        ...
        await tracker.SendAsync(login, ct);
        var loginAck = await tracker.ReceiveAsync(TimeSpan.FromSeconds(5), ct);
        ...
        await tracker.SendAsync(location, ct);
        var locationAck = await tracker.ReceiveAsync(TimeSpan.FromSeconds(5), ct);
        Assert.True(Gt06Packets.IsValidAck(locationAck, 0x12, 2), $"Некорректный ACK на локацию: {Convert.ToHexString(locationAck)}");

        var points = await _server.WaitForPointsAsync(Gt06Packets.Imei, count: 1, ct: ct);
        var point = Assert.Single(points);
        ...
        await tracker.SendAsync(Gt06Packets.Heartbeat(serial: 3), ct);
        var heartbeatAck = await tracker.ReceiveAsync(TimeSpan.FromSeconds(5), ct);
    ...
    public async Task Gt06_PacketWithCorruptedCrc_IsIgnored()
    {
        ...
        await tracker.SendAsync(Gt06Packets.Login(Gt06Packets.Imei, serial: 1), ct);
        await tracker.ReceiveAsync(TimeSpan.FromSeconds(5), ct);

        var corrupted = Gt06Packets.WithCorruptedCrc(
            Gt06Packets.Location(55.7512, 37.6184, speedKmh: 42, courseDegrees: 120, serial: 2));
        await tracker.SendAsync(corrupted, ct);

        var points = await _server.WaitForPointsAsync(Gt06Packets.Imei, count: 1, timeout: TimeSpan.FromSeconds(2), ct: ct);
